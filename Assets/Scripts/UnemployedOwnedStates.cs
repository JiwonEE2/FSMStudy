...
namespace UnemployedOwnedStates
{
	public class RestAndSleep : State<Unemployed>
	{
		...
		public override void Execute(Unemployed entity)
		{
			...
			if (entity.Bored >= 4)
			{
				// "이지원"에게 PC방에 함께 가자고 메시지 전송
				string receiver = "이지원";
				entity.PrintText(
					$"Send Message: {entity.EntityName}님이 {receiver}에게 GO_PCROOM 전송");
				MessageDispatcher.Instance.DispatchMessage(0, entity.EntityName,
					receiver, "GO_PCROOM");
	...
	public class HitTheBottle : State<Unemployed>
	{
		...
		public override void Execute(Unemployed entity)
		{
			entity.PrintText("취할 때까지 술을 마신다.");
