...
public class MessageDispatcher
{
	...
	public void DispatchMessage(float delayTime, string senderName,
		string receiverName, string message)
	{
		// 수신자 정보 검색
		BaseGameEntity receiver =
			EntityDatabase.Instance.GetEntityFromID(receiverName);
		// 존재하지 않는 에이전트이면 경고문 출력
		if (receiver == null)
		{
			Debug.Log(
				$"<color=red>Warning! No Receiver with ID of <b><i>{receiverName}</i></b> found</color>");
			return;
		}
	...
	public void DispatchDelayedMessages()
	{
		...
		foreach (KeyValuePair<float, Telegram> entity in prioritySD)
		{
			...
			{
				BaseGameEntity receiver =
					EntityDatabase.Instance.GetEntityFromID(entity.Value.receiver);

				// receiver에게 전보 전송
				Discharge(receiver, entity.Value);
				// 우선순위 Dictionary 자료구조에서 방금 보낸 전보 삭제
